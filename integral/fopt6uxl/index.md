---
url: /integral/fopt6uxl/index.md
---
# 高等微积分 Lesson 26
...
* （变上限积分求导）设$f\in C(\[a,b])$，令$F(x)=\int\_a^xf$，则$F'(x)=f(x)$，$\forall(a,b)$；在边界上，$F'(a+)=f(a)$，$F'(b-)=f(b)$.
  连续函数的变上限积分是其原函数.
...
> $$
> F'(\xi)=\frac{F(b)-F(a)}{b-a}=\frac{\int\_a^bf}{b-a}=f(\xi)
> $$
...
* （Newton - Leibiniz 公式）设$f\in\mathcal{R}(\[a,b])$，设$F'(x)=f(x)$，$\forall x\in(a,b)$，且==$F$在$\[a,b]$上连续==，则
  $$
  \int\_a^bf(x)\text{d}x=F|\_a^b=F(b)-F(a)
  $$
...
> $f\longrightarrow$ 积分$\int\_a^bf$ $\longrightarrow$ 求导$f$；
>
