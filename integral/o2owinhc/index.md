---
url: /integral/o2owinhc/index.md
---
# 高等微积分 Lesson 20

## 带 Peano 余项 Taylor 公式

设$f(x)$在$x\_0$处有$n$阶导数，则$f$在$x\_0$附近可近似为$n$次多项式，
$$
f(x)=f(x\_0)+\frac{f^{(1)}}{1!}(x-x\_0)+\cdots+\frac{f^{(n)}}{n!}(x-x\_0)^n+o((x-x\_0)^n)\text{ as }x\to x\_0
$$
问：如何判断$f$有高阶导数？
...
> $$
>
> 则：1. $f$在$\R$上有$n$阶导（$\forall n\in\Z\_+$）；2. $f^{(n)}(0)=0$，$\forall n\geq1$.

/Proof/

> $f$在$\R\_-$上恒为$0$ $\Longrightarrow$ $f^{(n)}$在$\R\_-$上恒为$0$.
>
> 在$\R\_+$上，$f(x)=e^{-\frac{1}{x^2}}=e^{-x^{-2}}$，得到：
> $$
> \begin{aligned}
> f(x)&=e^{-x^{-2}}\\
...
> /Claim/
>
> > $\exist$多项式$P\_n(y)$，使得在$\R\_+$上有
> > $$
> > f^{(n)}(x)=e^{-\frac{1}{x^2}}P\_n(\frac{1}{x})
> > $$
>
> /Proof/
>
...
> > $$
> > \begin{aligned}
> > f^{(n+1)}(x)&=e^{-x^{-2}}(2x^{-3})P\_n(\frac{1}{x})+e^{-x^{-2}}P'*n(\frac{1}{x})(-x^{-2})\\
> > &=e^{-x^{-2}}(2x^{-3}P\_n(\frac{1}{x})-x^{-2}P\_n'(\frac{1}{x}))\\
> > &=e^{-x^{-2}}P*{n+1}(\frac{1}{x})
> > \end{aligned}
> > $$
> > 得证.
>
...
> > $$
> > \begin{aligned}
> > f'(0+)&=\lim\_{x\to0+}\frac{f(x)}{x}=\lim\_{x\to0+}\frac{e^{-x^{-2}}}{x}\\\\
> > &\overset{x=1/y}{=}\lim\_{y\to+\infty}\frac{y}{e^{y^2}}=0
> > \end{aligned}
> > $$
> > 得到$f'(0)=0$.
> >
...
> > $$
> > \begin{aligned}
> > f^{(m+1)}(0+)&=\lim\_{x\to0+}\frac{e^{-x^{-2}}P\_m(\frac{1}{x})}{x}\\\\
> > &=\lim\_{y\to+\infty}\frac{yP\_m(y)(\text{polynomial})}{e^{y^2}(\text{exponential})}=0
> > \end{aligned}
> > $$
> > 得到$f^{(m+1)}(0)=0$. 得证.
>
...
> $$
> \begin{aligned}
> f(x)&=a\_1x^1+\cdots+a\_nx^n+o(x^n)\\\\
> g(x)&=b\_0+b\_1y+\cdots+b\_ny^n+o(y^n)
> \end{aligned}
> $$
...
> $$
> \begin{aligned}
> g(f(x))&=b\_0+\sum\_{k=1}^nb\_k(a\_1x+\cdots+a\_nx^n)^k+o(x^n)\text{ as }x\to0\\
...
> RHS 中，$x^m$（$m>1$）的系数为
> $$
> \sum\_{k=1}^nb\_k\sum\_{\underset{i\_1+\cdots+i\_k=m}{(i\_1,\cdots,i\_k)}}a\_{i\_1}\cdot a\_{i\_2}\cdot\cdots\cdot a\_{i\_k}
> $$
> 回忆，有
> $$
> a\_{i\_k}=\frac{f^{(i\_k)}(0)}{i\_k},,\quad b\_k=\frac{g^{(k)}(0)}{k!}
> $$
> 所以上面的系数是
> $$
> \sum\_{k=1}^n\frac{1}{k!}g^{(k)}(0)\sum\_{\underset{i\_1+\cdots+i\_k=m}{(i\_1,\cdots,i\_k)}}\frac{f^{(i\_k)}(0)g^{(k)}(0)}{i\_k!k!}\overset{\text{marked as}}{=}R
> $$
> 现在要验证，是否有
> $$
...
> \Longrightarrow g(y)&=\sum\_{k=0}^n\frac{\alpha(\alpha-1)\cdots(\alpha-k+1)}{k!}x^k+o(x^n)
> \end{aligned}
> $$
> 这里用到延拓的组合数概念：
> $$
...
> $$
> (1+y)^\alpha=1+\sum\_{k=1}^\infty\begin{pmatrix}
> \alpha\k
> \end{pmatrix}y^k
> $$
...
> $$
> \begin{aligned}
> f'(0)&=\lim\_{x\to0}\frac{f(x)-f(0)}{x}=\lim\_{x\to0}\frac{(1+x)^{1/x}-e}{x}\\\\
> &\overset{\text{L'H}\hat{\text{o}}\text{pital}}{=}\lim\_{x\to0}(1+x)^{1/x}(\frac{\frac{x}{1+x}-\ln(1+x)}{x^2})\\\\
> &=e\lim\_{x\to0}\frac{\frac{x}{1+x}-\ln(1+x)}{x^2}\longrightarrow \text{marked as }A\\\\
> &=e\lim\_{x\to0}\frac{x(1-x+o(x))-(x-\frac{x^2}{2}+o(x^2))}{x^2}\\\\
> &=-\frac{1}{2}e
> \end{aligned}
> $$
...
> $$
> \begin{aligned}
> f''(0)&=\lim\_{x\to0}\frac{f'(x)-f'(0)}{x}=\lim\_{x\to0}\frac{(1+x)^{1/x}\frac{\frac{x}{1+x}-\ln(1+x)}{x^2}+\frac{1}{2}e}{x}
> \end{aligned}
> $$
...
> f''(0)&=\lim\_{x\to0}\frac{(e-\frac{e}{2}x+o(x))(-\frac{1}{2}+\frac{2}{3}x+o(x))+\frac{1}{2}e}{x}\\\\
> &=\lim\_{x\to0}(\frac{1}{4}e+\frac{2}{3}e+o(x))=\frac{11}{24}e
> \end{aligned}
> $$
> 所以前面两阶 Taylor 公式为
> $$
...
> $$
> f(b)=f(a)+\sum\_{k=1}^{n-1}\frac{f^{(k)}(a)}{k!}(b-a)^k+\frac{f^{(n)}(\xi)}{n!}(b-a)^n
> $$
...
> /Proof/
>
> > 用一次 Rolle 定理，得到$\exist x\_1\in(a,b)$使得$h'(x\_1)=0$，又有$h'(a)=h'(x\_1)$，再次使用 Rolle 定理，之后递推到$n$阶，就能证明这一引理.
>
> 用引理证明 Taylor 公式：
>
> 构造
> $$
> h(x)=f(x)-(f(a)+\sum\_{k=1}^{n-1}\frac{f^{(k)}(a)}{k!}(x-a)^k)-\lambda(x-a)^n
> $$
> 显然有
> $$
> \begin{aligned}
...
> $$
> \begin{aligned}
> P(a+h)&=P(a)+\sum\_{k=1}^{n-1}\frac{P^{(k)}(a)}{k!}h^k\\\\
> \Longleftrightarrow P(b)&=P(a)+\sum\_{k=1}^{n-1}\frac{P^{(k)}(a)}{k!}(b-a)^k
> \end{aligned}
> $$
> 上面的式子有两种看法，可以将$b$作为主元，也可以将$a$作为主元. 我们采用第二种观点，冻结$b$，就会发现
> $$
> P(b)=P(x)+\sum\_{k=1}^{n-1}\frac{P^{(k)}(x)}{k!}(b-x)^k
> $$
> 下面用这种视角证明 Taylor 公式，令
> $$
> F(x)=f(x)+\sum\_{k=1}^{n-1}\frac{f^{(k)}(x)}{k!}(b-x)^k
> $$
> 则
> $$
> \begin{aligned}
> F'(x)&=f'(x)+\sum\_{k=1}^{n-1}\frac{f^{(k+1)}(x)}{k!}(b-x)^k-\sum\_{k=1}^{n-1}\frac{f^{(k)}(x)}{(k-1)!}(b-x)^{k-1}\\\\
...
> 所以
> $$
> f(b)=\sum\_{k=0}^{n-1}\frac{f^{(k)}(a)}{k!}(b-a)^k+\frac{\frac{f^{(n)}(\xi)}{(n-1)!}(b-\xi)^{n-1}}{g'(\xi)}(g(b)-g(a))
> $$
...
> $$
> R\_n=f(x)-\sum\_{k=1}^n\frac{f^{(k)}(0)}{k!}x^k=\frac{f^{(n+1)}(\xi)}{(n+1)!}x^{n+1}
> $$
> 显然这个$\xi$依赖于$0$和$x$，记为$\xi\_n$. 现在我们要证明$\underset{n\to\infty}{\lim}R\_n=0$.
>
> 注意到，
> $$
> |R\_n|=\frac{e^{\xi\_n}}{(n+1)!}|x|^{n+1}\leq\frac{\max{e^0,e^x}}{(n+1)!}|x|^{n+1}\to0
> $$
> （这里用到$e^{\xi\_n}$有界）
