---
url: /quantum-mechanics/lesson-4-oscillator/index.md
---
上节课讲的是 Schrödinger 方程的一些基本的概念，说到 2-state superposition：

> $$
> |\varPsi(x,t)|^2=c\_1^2\psi\_1^2+c\_2^2\psi\_2^2+2c\_1c\_2\psi\_1\psi\_2\cos\left(\frac{E\_2-E\_1}{\hbar}t\right)
> $$
...
> $$
> \int\_0^a\psi\_m^*(x)\psi\_n(x)\text{d}x=0,,\quad(m\neq n)
> $$
> 归一性：
> $$
> \int\_0^a|\psi\_n(x)|^2\text{d}x=1
> $$
> 原则上任何函数都可以在这一组基上展开，
> $$
> f(x)=\sum\_{n=1}^\infty c\_n\psi\_n(x),,\quad\text{Fourier expansion}
> $$
> 这是一定意义上的 completeness. 当然真正的完备性应该是
> $$
> \sum\_{n=0}^\infty\psi\_m^*(x)\psi\_n(x')=\delta(x-x')
> $$
> 如果已经知道初态的波函数 $\varPsi(x,0)$，那么
> $$
> \begin{aligned}
> \varPsi(x,0)&=\sum\_{n=1}^\infty c\_n\psi\_n(x)\\\\
> \varPsi(x,t)&=\sum\_{n=1}^\infty c\_n\psi\_n(x)e^{-\text{i}E\_nt/\hbar},,\quad c\_n=\sqrt{\frac{2}{a}}\int\_0^a\sin\left(\frac{n\pi x}{a}\right)\varPsi(x,0)\text{d}x
> \end{aligned}
> $$
> 能量平均值：
> $$
> \braket{H}=\int\_0^a\varPsi^\*(x,t)\hat{H}\varPsi(x,t)\text{d}x=\sum\_{n=1}^\infty |c\_n|^2E\_n
> $$

一个简单的例子是 $\varPsi(x,0)=Ax(a-x)$，那么
$$
A=\sqrt{\frac{30}{a^5}},,\quad c\_n=\begin{cases}
