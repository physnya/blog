---
url: /quantum-mechanics/lesson-8-potential-well/index.md
---
今天主要是做几个题目.

先把上节课没讲完的说一下，自由粒子的波函数可能出现色散，具体而言是
$$
\omega(k) \approx \omega\_0 + \omega\_0'(k-k\_0)
$$
则得到波函数
$$
\psi(x,t)\approx\frac{1}{\sqrt{2\pi}}\int\_{-\infty}^\infty\phi(k\_0+s)e^{\text{i}(k\_0+s)(x-\omega\_0't)}\text{d}s
$$
...
* $E<0$，则
  ...
  $$
  -\int\_{-\varepsilon}^\varepsilon\frac{\hbar^2}{2m}\frac{\text{d}^2\psi}{\text{d}x^2}\text{d}x-\int\_{-\varepsilon}^\varepsilon\alpha\delta(x)\psi(x)\text{d}x=\int\_{-\varepsilon}^\varepsilon E\psi(x)\text{d}x
  $$
  得到
  $$
...
$$
\psi(x)=C\sin(lx)+D\cos(lx),,\quad l =\sqrt{\frac{2m(E+V\_0)}{\hbar^2}}
$$
...
令
$$
z=la,,\quad z\_0=\frac{a}{\hbar}\sqrt{2mE}
$$
方程变为
$$
\tan z=\sqrt{\left(\frac{z\_0}{z}\right)^2-1}
$$
