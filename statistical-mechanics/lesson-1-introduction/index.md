---
url: /statistical-mechanics/lesson-1-introduction/index.md
---
::: warning
...
> A 和 C 平衡：
> $$
> f\_{AC}(p\_A,V\_A;p\_c,V\_c) = 0\Longrightarrow p\_C = F\_{AC}(p\_A,V\_A;V\_C)
> $$
> B 和 C 平衡：
> $$
> f\_{BC}(p\_B,V\_B;p\_c,V\_c) = 0\Longrightarrow p\_C = F\_{BC}(p\_B,V\_B;V\_C)
> $$
> 所以
> $$
> F\_{AC}(p\_A,V\_A;V\_C) = F\_{BC}(p\_B,V\_B;V\_C)
> $$
> 利用热力学第零定律，A 和 B 也平衡，有 $f\_{AB}(p\_A,V\_A;p\_B,V\_B)=0$. 把上面一个方程的 $V\_C$ 看作一个常量，发现存在一个只和系统本身的参量有关的量：
> $$
> g\_A(p\_A,V\_A) = g\_B(p\_B,V\_B)
> $$
