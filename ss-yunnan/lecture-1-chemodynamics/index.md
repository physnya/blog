---
url: /ss-yunnan/lecture-1-chemodynamics/index.md
---
::: right
...
$$
\frac{\mathrm{d}(Zf\_g)}{\mathrm{d}t} = \underbrace{E\_{\text{SW}}+E\_{\text{SNcc}}+E\_{\text{SNIa}}}*{\text{Metal ejection rates}}\underbrace{\underset{\text{decreased by star evolution}}{\underline{-Z\psi}}\underset{\text{inflow}}{\underline{+Z*{\text{inflow}}}}\underset{\text{outflow}}{\underline{-ZR\_{\text{outflow}}}}}\_{\text{Galaxy Evolution}}
$$
...
一颗星，初始质量 $m$，lifetime $\tau\_m$，剩下的质量 $w\_m$，在时刻 $(t-\tau\_m)$ 出生、在时刻 $t$ 死亡. 终止质量 ($\tau\_m=t$) 为 $m\_t$.

$$
E=\int\_{m\_t}^{m\_u}(1-w\_m)\psi(t-\tau\_m)\phi(m)\mathrm{d}m
$$

大于终止质量的星体是死掉的星体. SNe 由 nucleosynthesis yields (核合成产率) $p\_{zm}$ 给出，

$$
E\_{Z,\text{cc}} = \int\_{m\_t}^{m\_u}p\_{zm}\psi(t-\tau\_m)\phi(m)\mathrm{d}m
$$
...
* Star formation rate: $\psi\propto f\_g$
* Inflow rate: $R\_{\text{in}}\propto e^{-t/\tau}$ or $te^{-t/\tau}$
* Outflow rate: $R\_{\text{out}}\propto\psi$ (SN-driven) or $\psi=0$ for $t>t\_w$ (AGN-driven)
...
初始情况都是 $f\_{g,0}=1$. 三种模型的演化曲线有很大的差异. 我们只讨论 one-zone model 情况下的结论，从观测数据来看，$\[\text{O}/\text{Fe}]$ - $\[\text{Fe}/\text{H}]$ 关系有一个开始时比较慢、之后比较快的下降，拟合最好的模型是 infall model.
...
* GPT's Explanation
  ...
  *Collapsars*
  ...
  Currently the strongest competitor. The basic idea is a very massive star $M\gtrsim25\sim30,M\_\odot$ collapses directly into a black hole. An accretion disk forms.
  ...
  * Long Gamma-Ray Bursts (GRBs)
  ...
  > **Advantages**
  >
  > Produces enormous r-process mass. Typically $0.05-0.3,M\_\odot$ per event, much larger than many core-collapse supernovae. Occurs immediately after star formation.
  >
  > Can explain
  >
  > * early Eu enrichment
  ...
  *Neutrino-driven Winds*
  ...
  > **Problems**
  >
  > Modern simulations generally find $Y\_e$ is too high.
  >
  ...
  *Electron-Capture Supernovae (ECSNe)*
  ...
  Occurs in stars of roughly $8-10,M\_\odot.$
...
* $\nabla\cdot\vec{v}<0$
* $t\_{\text{cool}}\<t\_{\text{dyn}}$
* $t\_{\text{dyn}}\<t\_{\text{sound}}$
...
$$
m\_i\frac{\mathrm{d}^2x\_i}{\mathrm{d}t^2}=\sum\_{j\neq i}Gm\_im\_j\frac{x\_j-x\_i}{|x\_j-x\_i|^3}
$$
...
* <https://arxiv.org/abs/2506.20436>
* <https://arxiv.org/abs/2302.07255>
* <https://arxiv.org/pdf/1802.03353v1>
* <https://arxiv.org/pdf/1012.5144>
