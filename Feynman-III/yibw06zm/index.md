---
url: /Feynman-III/yibw06zm/index.md
---
## 2024-12-25
...
Feynman 发现，将上面两个图加起来得到一个 Lorentz 不变的散射幅. 同时再乘上发射光子和吸收光子的概率幅，得到上面一幅图的总概率幅，是$\frac{Q\_1Q\_2}{E^2-\vec{k}^2}$.
...
但是到这一步还是不能说明引力过程中任何粒子的引力荷相同. 在引力子的传递中，发射、吸收概率幅为$\kappa\_1$、$\kappa\_2$，同时因为自旋 2 的 Green 函数具有某些张量结构导致惯性质量的出现，最后得到的结果应该是

$$
\frac{\kappa\_1\kappa\_2m\_1m\_2}{r}
$$

但是 Feynman 无法给出$\kappa\_1=\kappa\_2$的解释（所有粒子以同样的强度参与引力作用）.
...
$$
\epsilon\_x^\mu=(0,1,0,0),,\epsilon\_y^\mu=(0,0,1,0),,\epsilon\_z^\mu=(0,0,0,1)
$$
...
第 2 点来源：保持$p^\mu=(p,0,0,p)$不变的 Lorentz 变换构成一个所谓的 ISO(2,1) 群，这个群是二维的旋转变换加上两个平移变换. 这导致光子只有两种极化，光的极化矢量不是 Lorentz 矢量，因此有规范对称$\epsilon\_\mu^{(1)}-\epsilon\_\mu^{(2)}\propto p^\mu$，视为等同.
...
$$
\frac{Q\_i(\epsilon\_\mu p\_i^\mu)}{(p\_i-q)^2-m^2\_i}
$$

取$q\to0$的极限，粒子趋于在壳，$p\_i\sim m\_i$，这个过程的概率幅线性发散，入射概率幅是

$$
\frac{Q\_i(\epsilon\cdot p\_i)}{2(p\_i\cdot q)}
$$

对于 Case 2：出射粒子动量$p\_j$，出射光子动量$q$，对应的散射幅在$q\to0$极限下，散射幅正比于

$$
\frac{Q\_j(\epsilon\cdot p\_j)}{2(p\_j\cdot q)}
$$
...
$$
\mathscr{M}=M\_0\times{\sum\_{i\in\text{input}}\frac{-Q\_i(\epsilon\cdot p\_i)}{2p\_i\cdot q}+\sum\_{j\in\text{output}}\frac{Q\_j(\epsilon\cdot p\_j)}{2p\_j\cdot q}}
$$
...
$$
\sum\_{i\in\text{in}}Q\_i=\sum\_{j\in\text{out}}Q\_j
$$
...
$$
M\_0\times{-\sum\_{i\in\text{in}}\frac{\kappa\_i\epsilon\_{\mu\nu}p\_i^\mu p\_j^\nu}{2(p\_i\cdot q)}+\sum\_{j\in\text{out}}\frac{\kappa\_j\epsilon\_{\mu\nu}p\_j^\mu p\_i^\nu}{2(p\_j\cdot q)}
}=0
$$
...
$$
\sum\_{i\in\text{in}}\kappa\_ip\_i^\mu=\sum\_{j\in\text{out}}\kappa\_jp\_j^\mu,,\quad\sum\_{i\in\text{in}}p\_j^\mu=\sum\_{j\in\text{out}}p\_j^\mu
$$

这里有解必须要求$\kappa\_i=\kappa\_j$，所以只有一种用自旋 - 2 的无质量粒子传递的相互作用，所有粒子以相同的强度参与这种作用. 同时，不存在以自旋$>2$的无质量粒子传递的相互作用，因为方程会无解，这些相互作用不自洽.
