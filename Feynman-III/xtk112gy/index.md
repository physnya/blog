---
url: /Feynman-III/xtk112gy/index.md
---
## 2024-12-02
...
$$
\[(\hat{x}\times\hat{y})\cdot\hat{z}]=\varepsilon\_{ijk}\hat{x}\_i\hat{x}\_j\hat{x}\_k
$$
也就是赝标量.

可以发现，讨论宇称时，我们往往要引入全反对称张量（$\varepsilon\_{i\_1i\_2\cdots i\_d}$），这个量能够辨别宇称变换. 使用这个量，我们能够在$d$维空间构造一个赝标量$\varepsilon\_{i\_1\cdots i\_d}\hat{x}*{i\_1}\cdots\hat{x}*{i\_d}$. 当赝标量反号就是宇称.

其实宇称的定义是和空间本身的性质有关系的，试想我们如何在一个 Mobius 环上定义宇称？这件事情并不简单. 我们定义的宇称要求空间是“可定向”的（想想可定向流形）.

事实上，宇称的重要性在于只有弱作用中宇称不守恒：$H\_{\text{weak}}P\neq PH\_{\text{weak}}$. 这应当是常识的一部分，结论由李-杨给出.
...
$\Lambda^0$由 u、d、s 三个 quark 组成，自旋$-1/2$，$m\_{\Lambda^0}=1116\text{MeV}$（在 Feynman 的时代这还不能确定，但是我们现在能够肯定地写出这个数值）.
...
对称性$\Longrightarrow$守恒律. 这里对称性体现为和$H$对易，也就是说对称变换$Q$满足$\[Q,H]=0$. 取$Q$的本征态$\ket{t\_0}$，得到$Q\ket{t\_0}=e^{i\delta}\ket{t\_0}$，由对易关系得到$\[Q,U(t,t\_0)]=0$.

故
$$
Q\ket{t}=QU(t,t\_0)\ket{t\_0}=U(t,t\_0)Q\ket{t\_0}=e^{i\delta}U(t,t\_0)\ket{t\_0}=e^{i\delta}\ket{t\_0}
$$
...
$$
\begin{aligned}
\braket{\psi(t)|Q|\psi(t)}&=\braket{\psi(t\_0)|U^\dagger QU|\psi(t\_0)}\\\\
&=\braket{\psi(t\_0)|Q|\psi(t\_0)}=\braket{Q}\_t
\end{aligned}
$$
这也是一个守恒的量.
...
一个简单的例子是绕$z$轴的转动对称性，看$R\_z(\theta)$的本征态$\ket{m}$，有$R\_z(\theta)\ket{m}=e^{i\delta(\theta)}\ket{m}$，这里$\delta$必须线性，假设系数是$m$，则有$\delta(\theta)=m\theta$，$m$守恒.
...
另一个例子是空间平移$x\to x+a$，本征态$T\_x(a)\ket{k}=e^{i\delta(a)}\ket{k}$，所以$\delta(a)=ka$（因为线性），这显然是动量守恒. 同时值得注意的是，空间平移是 Abelian 群，因为任意群元可交换.
