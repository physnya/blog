---
url: /equation-m-p/lesson-13/index.md
---
## 内积空间与函数空间
...
函数内积定义的推广：
$$
(f\_1,f\_2) = \int f\_1(x)f\_2(x)\rho(x)\text{d}x
$$
Hilbert 空间的任意函数可以按照相应正交完备基展开：
$$
f(x) = \sum\_{i=1}^\infty c\_if\_i(x)
$$
展开系数是唯一的.
...
基本空间有很多具体要求，例如如果是一个函数序列 ${\varphi\_n(x)}$ 构成基本空间，要求 $\varphi\_n(x)\in C^\infty$ 以及任意阶导数构成的序列趋于零等等.
...
> * $\delta'$ 是奇函数；
>
> * $\displaystyle{\int\_{-\infty}^\infty\delta(t)}\text{d}t=\theta(x)$ (阶跃函数)
>
> * 有关系：$g(x)\delta(0)=g(0)\delta(x)$.
>
>   > 原则上不应该取 $g(x)=0$ 这种，因为之后可能还要做一些操作或者其他计算，有 $g(x)=0$ 会产生一些影响.
>
> * $\displaystyle{\delta\[g(x)] = \frac{\delta(x-x\_0)}{|g'(x\_0)|}}$，其中 $g(x\_0)=0$ 且 $g'(x\_0)\neq0$.
>
> 一个神必小技巧是，$\delta(x)$ 自身的变换非常像 $(\text{d}x)^{-1}$ 的变换.

$\delta$ 函数的一个重要形式：
$$
\delta(x)=\frac{1}{2\pi}\int\_{-\infty}^\infty e^{\text{i}kx}\text{d}k
$$
...
> \&c\_1x^l+d\_1x^{-l-1},&\quad x\<c\\\\
> \&c\_2x^l+d\_2x^{-l-1},&\quad x>c
> \end{aligned}\right.
> $$
> 在 $x=c$ 处，函数本身连续，导数满足：
> $$
> \int\_{c^-}^{c^+}\frac{\text{d}}{\text{d}x}\left(x^2\frac{\text{d}y}{\text{d}x}\right)\text{d}x = c^2 =(x^2y')\big|^{c^+}\_{c^-}
> $$
> 和边界条件一起得到四个方程，可解.
...
> 即
> $$
> \int\_a^bv^\*\bold{L}u\text{d}x=\int\_a^b(\bold{M}v)^\*u\text{d}x
> $$
...
> 由于 $\bold{L}$ 自伴，
> $$
> \int\_a^b\[y^*\bold{L}y-(\bold{L}y)^*y]\text{d}x=0\Longrightarrow(\lambda-\lambda^*)\int\_a^byy^*\text{d}x=0
> $$
...
> 与如下泛函
> $$
> \lambda\[y] = \frac{\displaystyle{\int\_a^by^*(x)\bold{L}y(x)\text{d}x}}{\displaystyle{\int\_a^by^*(x)y(x)\text{d}x}}
> $$
...
取有限函数集 ${x^k;k=0,1,\cdots,n}$，试探函数 $y(x)=\displaystyle{\sum\_{k=0}^nc\_kx^k}$，计算泛函值：
$$
\lambda\[c\_k]=-\left.\int\_0^1y(x)y''(x)\text{d}x\right/\int\_0^1y^2(x)\text{d}x
$$
...
边界条件 (第三类)：
$$
p(x)\left.\left(y\_1^*\frac{\text{d}y\_2}{\text{d}x}-y\_2\frac{\text{d}y\_1^*}{\text{d}x}\right)\right|^b\_a=0
$$
