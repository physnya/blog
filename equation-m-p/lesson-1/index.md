---
url: /equation-m-p/lesson-1/index.md
---
...
$$
\begin{aligned}
(T\cos\theta)*{x+\text{d}x}-(T\cos\theta)*x&=0\\
(T\sin\theta)*{x+\text{d}x}-(T\sin\theta)*x&=\rho\text{d}x\cdot\frac{\partial^2u}{\partial t^2}
\end{aligned}
$$
...
$$
T\left(\left.\frac{\partial u}{\partial x}\right|*{x+\text{d}x}-\left.\frac{\partial u}{\partial x}\right|*{x}\right)=\rho\text{d}x\cdot\frac{\partial^2u}{\partial t^2}
$$
也就是
$$
...
> >
> > 线性一定有叠加原理，可以把 $u$ 换成两个特解的线性叠加 $c\_1u\_1+c\_2u\_2$.
>
> 什么是「齐次」？
>
...
$$
\begin{aligned}
x\text{ - axis}:&\[(q\_x)*x-(q\_x)*{x+\text{d}x}]\Delta y\Delta z\Delta t\\\\
...
|                    波动方程                    |      波动       |  双曲型  |
|                   热传导方程                   |      扩散       |  抛物型  |
| Poisson 方程Laplace 方程Helmholtz 方程 |      稳恒       |  椭圆型  |
...
如果边界上是一个弹簧，那么边界条件是
$$
\left\[\frac{\partial u}{\partial x}+\frac{k}{E}u\right]\_{x=l}=\frac{k}{E}u\_0
$$
...
也有时是边界上的热流已知：
$$
\left.\frac{\partial u}{\partial n}\right|*\Sigma=\frac{1}{k}\psi(\Sigma,t)
$$
...
>
> 以齐次初始条件 $y'|*{x=0}=y|*{x=0}=0$ 定解，方程只有零解；以齐次边界条件 $y|*{x=0}=y|*{x=\pi}=0$ 定解，方程有非零解 $y=C\sin x$，齐次边界条件能够给出非零解.
>
...
> u|*{y=0}=u|*{y=l}=0\&x\geq0\\\\
> u|*{x=0}=\frac{\partial u}{\partial x}|*{x=0}=0&0\leq y\leq l
> \end{array}\right.
> $$
...
* 常系数方程
  ...
  得到关于 $D$ 的多项式乘上 $y$，比如
  $$
  (D+\alpha\_1)(D+\alpha\_2)y=0
  $$
  最终的解是 $y=e^{-\alpha\_1},e^{-\alpha\_2}$.
...
* 常系数非齐次方程，可以用常数变易法找一个特解.
  ...
  y''+by'+cy=f
  $$
  通解是 $C\_1y\_1+C\_2y\_2$，求一阶导
  $$
  C\_1y\_1'+C\_2y\_2'+C\_1'y\_1+C\_2'y\_2
  $$
  因为只要找特解，所以可以强行要求后面两项和为零，有
  $$
  \begin{cases}
  C\_1'y\_1+C\_2'y\_2=0\\\\
  C\_1'y\_1'+C\_2'y\_2'=f
  \end{cases}
  $$
  定出函数系数.
